...
namespace EpgTimer
{
    ...
    public partial class EpgContentData : ICtrlCmdReadWrite, IDeepCloneObj
    {
        ...
        public static void FixNibble(List<EpgContentData> list)
        {
            if (list == null) return;
            foreach (var info in list)
            {
                if (info.content_nibble_level_1 == 0x0E && info.content_nibble_level_2 == 0x00)
                {
                    info.content_nibble_level_1 = 0x0F;
                    info.content_nibble_level_2 = 0x0F;
                }
            }
        }
